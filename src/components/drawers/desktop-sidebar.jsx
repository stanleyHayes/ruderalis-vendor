...
const SectionLabel = ({children, collapsed}) => {
    if (collapsed) return <Divider sx={{mx: 2, my: 1, opacity: 0.3}}/>;
    return (
        <Typography
            variant="caption"
            sx={{
...
const DesktopSidebar = ({collapsed = false}) => {
    ...
    return (
        <Box sx={{height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden'}}>
            {/* Logo */}
            <Stack sx={{px: collapsed ? 1.5 : 2.5, pt: 2.5, pb: 2, justifyContent: collapsed ? 'center' : 'flex-start'}} direction="row" alignItems="center" spacing={collapsed ? 0 : 1.5}>
                <Link to="/" style={{textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', width: collapsed ? '100%' : 'auto'}}>
                    <img src={logo} style={{width: collapsed ? 30 : 34, height: collapsed ? 30 : 34, objectFit: 'contain'}} alt="Ruderalis Logo"/>
                </Link>
                {!collapsed && (
                    <>
            ...
            <Divider sx={{mx: collapsed ? 1 : 2, opacity: 0.4}}/>
            ...
            <Box sx={{flex: 1, overflow: 'auto', pb: 1}}>
                ...
                <SectionLabel collapsed={collapsed}>Sales & Customers</SectionLabel>
            ...
            {/* Bottom Actions */}
            <Box sx={{px: collapsed ? 0.5 : 1.5, pb: 1}}>
                <Divider sx={{mb: 1, opacity: 0.4}}/>

                {collapsed ? (
                    <Stack alignItems="center" spacing={0.5}>
                        <Tooltip title={themeVariant === 'dark' ? 'Light Mode' : 'Dark Mode'} placement="right">
                            <Box onClick={() => dispatch(toggleTheme())} sx={{width: 40, height: 40, borderRadius: 1.5, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', '&:hover': {bgcolor: 'light.gray'}}}>
                                {themeVariant === 'dark' ? <LightModeRounded sx={{fontSize: 18, color: '#FFA726'}}/> : <DarkModeRounded sx={{fontSize: 18, color: '#5C6BC0'}}/>}
                            </Box>
                        </Tooltip>
                        <Tooltip title="Logout" placement="right">
                            <Box onClick={handleLogout} sx={{width: 40, height: 40, borderRadius: 1.5, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', '&:hover': {bgcolor: 'rgba(239,83,80,0.06)'}}}>
                    ...
                    <>
                        <Stack onClick={() => dispatch(toggleTheme())} direction="row" spacing={1.5} alignItems="center"
                            sx={{px: 2, py: 0.8, borderRadius: 2, cursor: 'pointer', '&:hover': {backgroundColor: 'light.gray'}}}>
                            {themeVariant === 'dark' ? <LightModeRounded sx={{fontSize: 18, color: '#FFA726'}}/> : <DarkModeRounded sx={{fontSize: 18, color: '#5C6BC0'}}/>}
                            <Typography variant="body2" sx={{color: 'text.secondary', fontSize: '0.82rem'}}>{themeVariant === 'dark' ? 'Light Mode' : 'Dark Mode'}</Typography>
                        </Stack>
                        <Stack onClick={handleLogout} direction="row" spacing={1.5} alignItems="center"
                            sx={{px: 2, py: 0.8, borderRadius: 2, cursor: 'pointer', '&:hover': {backgroundColor: 'rgba(239,83,80,0.06)'}}}>
                            <LogoutRounded sx={{fontSize: 18, color: orange[700]}}/>
                            <Typography variant="body2" sx={{color: orange[700], fontSize: '0.82rem', fontWeight: 500}}>Logout</Typography>
                        </Stack>
                    </>
                )}
            </Box>
            ...
            {/* User Card */}
            <Box sx={{px: collapsed ? 0.5 : 1.5, pb: 2}}>
                {collapsed ? (
                    <Tooltip title={user?.fullName || user?.companyName || 'Profile'} placement="right">
                        <Box component={Link} to="/profile" sx={{display: 'flex', justifyContent: 'center'}}>
                            <Avatar sx={{width: 34, height: 34, bgcolor: 'primary.main', fontSize: '0.85rem', fontWeight: 600, textDecoration: 'none'}}>
                                {user?.fullName?.[0] || user?.companyName?.[0] || 'R'}
                            </Avatar>
                        </Box>
                    </Tooltip>
                ) : (
                <Stack
                    ...
                    sx={{px: 2, py: 1.5, borderRadius: 2, backgroundColor: 'light.green', textDecoration: 'none', transition: 'all 0.15s ease', '&:hover': {opacity: 0.85}}}>
